...
class TicketManager {
    ...
    getEventById(idEvent) {
        const found = this.events.find(event => event.id === parseInt(idEvent))
    ...
    addNewEvent(idEvent, city, date) {
        ...
        if (event !== []) {
            ...
            console.log(`Nuevo id ${newEvent.id}`);
    ...
    editEvent(idEvent, objEventProps) {
        ...
        if (event !== [] && event) {
            ...
            console.log(`El evento con ID ${idEvent} no pudo modificarse porque no se encontró`);
        }
